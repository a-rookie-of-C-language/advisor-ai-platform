...
public class UserBehaviorFilter implements RiskFilter {
    ...
    public RiskCheckResponse check(RiskCheckRequest request) {
        ...
        // 检查历史违规次数
        long violationCount = userViolationRepository.countByUserIdSince(
                request.getUserId(), LocalDateTime.now().minusDays(30));

        if (violationCount >= permanentThreshold) {
            log.warn("User behavior violation threshold exceeded: userId={}, violationCount={}",
                    request.getUserId(), violationCount);
        ...
        // 检查今日行为统计
        Optional<UserBehaviorStat> statOpt = userBehaviorStatRepository.findByUserIdAndDate(
        ...
        if (statOpt.isPresent()) {
            ...
            if (stat.getSuspiciousPattern() != null && !stat.getSuspiciousPattern().isEmpty()) {
                log.warn("Suspicious behavior pattern detected: userId={}, pattern={}",
                        request.getUserId(), stat.getSuspiciousPattern());
                return RiskCheckResponse.builder()
                        .passed(false)
                        .action("warn")
                        .reason("检测到可疑行为模式")
