...
pub async fn health(State(state): State<AppState>) -> (StatusCode, Json<serde_json::Value>) {
    ...
    components.insert(
        "redis".to_string(),
        json!(if redis_ok { "ok" } else { "error" }),
    );
    match pg_ok {
        Some(ok) => {
            components.insert(
        ...
        None => {
            components.insert("postgres".to_string(), json!("not_configured"));
        }
    }
    components.insert(
        "provider".to_string(),
        json!(if provider_ok { "ok" } else { "error" }),
    );
    ...
    (
        code,
        response::ok(json!({"status": status, "components": components})),
    )
}
