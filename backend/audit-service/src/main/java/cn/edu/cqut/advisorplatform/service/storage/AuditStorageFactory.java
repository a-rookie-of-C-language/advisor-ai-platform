    ...
    public AuditStorageFactory(List<AuditLogStorage> storageList) {
        this.storages = storageList.stream()
                .collect(Collectors.toMap(
                        AuditLogStorage::storeType,
                        Function.identity()
                ));
        storageList.forEach(s -> log.info("Registered AuditLogStorage: {}", s.storeType()));

        this.defaultStorage = storages.values().stream().findFirst()
                .orElseThrow(() -> new IllegalStateException("No AuditLogStorage implementation found"));
    }
