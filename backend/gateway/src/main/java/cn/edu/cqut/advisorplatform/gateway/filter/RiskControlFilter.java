...
public class RiskControlFilter implements GlobalFilter, Ordered {
    ...
    private static final List<String> RISK_CHECK_PATHS = List.of(
            "/api/chat/",
            "/api/session/",
            "/api/rag/",
            "/api/memory/"
                ...
                .flatMap(dataBuffer -> {
                    ...
                    return callRiskControlService(userId, sessionId, ipAddress, path, requestBody)
                            .flatMap(response -> {
                                ...
                                log.warn("Risk control blocked: userId={}, path={}, category={}, reason={}",
                                        userId, path, response.getCategory(), response.getReason());
